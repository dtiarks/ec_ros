...
void
align::normalEstimation(pcl::PointCloud<pcl::PointXYZ>::Ptr &scene, pcl::PointCloud<pcl::PointNormal>::Ptr &normals) {
...
void align::featureEstimation(pcl::PointCloud<pcl::PointXYZ>::Ptr &input,
                              pcl::PointCloud<pcl::PointNormal>::Ptr &input_normals,
...
void align::euclideanSegment(const pcl::PointCloud<pcl::PointXYZ>::ConstPtr &cloud,
...
uint32_t align::segmentColor(int segment) {
    ...
    uint32_t rgb = ((uint32_t) rgbs[segment % SEGMENT_COLORS][0] << 16 |
                    (uint32_t) rgbs[segment % SEGMENT_COLORS][1] << 8 | (uint32_t) rgbs[segment % SEGMENT_COLORS][2]);
